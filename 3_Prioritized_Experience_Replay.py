...
class PER:
	def __init__(self):
		...
		# Game Information
		self.algorithm = 'PER'    #深度强化学习使用算法：优先经验回放
		...
		self.replay_memory = []    #经验回放池

		# Parameter for Target Network
		self.Num_update_target = Deep_Parameters.Num_update

		# Parameters for network
		self.img_size = 80    #输入图像大小
		self.Num_colorChannel = Deep_Parameters.Num_colorChannel #数据通道数
                #卷积神经网络模型架构参数设置
		...
		self.GPU_fraction = Deep_Parameters.GPU_fraction   #网络模型训练分配多少GPU运算
		...
		# Initialize Network
		self.input, self.output = self.network('network')               #获取在线网络输入输出数据格式
	...
	def main(self):
		...
		while True:
			...
			if self.progress == 'Training':
				# Update target network   更新目标网络
				if self.step % self.Num_update_target == 0:
					self.update_target()

				# Make mini-batch with Prioritization  优先经验回放   对数据进行选择  
	...
	def init_sess(self):
		...
		config = tf.ConfigProto()#创建session的时候,用来对session进行参数配置
		config.gpu_options.per_process_gpu_memory_fraction = self.GPU_fraction#程序最多只能占用指定gpu的self.GPU_fraction显存  

		sess = tf.InteractiveSession(config=config)

		# Make folder for save data  用于递归创建目录
	...
	def loss_and_train(self):
		...
		TD_error_tf = tf.subtract(y_prediction, y_target)# tf.subtract减法运算

		# Loss = tf.reduce_mean(tf.square(y_prediction - y_target))
		Loss = tf.reduce_sum(tf.multiply(w_is, tf.square(y_prediction - y_target)))#损失函数=网络的目标Q值-当前状态的Q值
		###################################################################################################################
	...
	def select_action(self, stacked_state):
		...
		if self.progress == 'Exploring':#搜索策略状态
			...
			action_index = random.randint(0, self.Num_action-1)#随即动作
	...
	def train(self, minibatch, w_batch, batch_index):
		...
		# Get target values状态动作值Q(s,a)的更新
